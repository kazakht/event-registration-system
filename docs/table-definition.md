...
| 開催日時 | EventDateTime | datetime2(7) | - | ○ | - | イベント開催日時 |
...
**インデックス:**
- IX_Events_Name (Name)
- IX_Events_EventDateTime (EventDateTime)
...
- SQL Server の `SYSDATETIME()` 関数でサーバーのローカル時刻を使用
  - **前提条件**: SQL Server のサーバー時刻は JST に設定されている必要があります
  - 他のタイムゾーンで運用する場合は、UTC（`SYSUTCDATETIME()`）で保存しアプリケーション側で表示時に変換する方式への変更を推奨します
- アプリケーション側でもタイムゾーン変換は不要（サーバー時刻がJSTの場合）
